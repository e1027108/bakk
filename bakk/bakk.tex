...
Regarding the relation between grounded and complete extensions, Dung [X] provides the following information:\hl
...
There also is an alternative semantic to the grounded extension. Ideal extension semantics as mentioned by Gorogiannis and Hunter [X] and Baronia and Giacomin [X] were introduced by Alferes, Dung and Peireira [X] and also produce a unique sceptical set, albeit a less strict one.\hl %zu viel?
...
\begin{align}
	gr(F)\subset id(F)\\ %baronia and giacomin
	id(F)\in co(F) %gorogiannis and hunter
\end{align}\cl
