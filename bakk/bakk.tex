...
\begin{abstract*}
This bachelor thesis explains argumentation frameworks and provides their most important semantics. It further analyzes the relations between different types of extension semantics.\hl %TODO still not good enough
...
\textbf{Definition 5.} Let $S$ be a conflict-free set. It is called an \emph{admissible set} if it defends each $a\in S$.\\
The set of all admissible sets of an argumentation framework $F$ is denoted $adm(F)$.\cl

\textbf{Example 3.} (Continuation of Example 2.) Of the conflict-free sets only \(\emptyset\) and $\{a_2\}$ don't get attacked. Therefore they are admissible sets. $\{a_1\}$ defends itself from its only attacker $b$ via the attack relation $(a_1,b)$, making it an admissible set. $\{a_1,a_2\}$ gets attacked via the attack relation $(b,a_1)$, but $a_1$ gets defended through $(a_1,b)$ and $(a_2,b)$, also making it an admissible set.\\
...
Most given argumentation frameworks produce big numbers of admissible sets. Preferred semantics reduce the number of sets, only taking the biggest ones. The resulting preferred extensions represent a credulous approach of reasoning, since they contain all arguments that are contained in admissible sets.\hl
They were defined in \cite{Egly} as follows:\cl

\textbf{Definition 6.} Let $S$ be an admissible set. It is called a \emph{preferred extension} if for each $S'\subseteq A$, that is an admissible set, $S\not\subset S'$.\\
The set of all preferred extensions of an argumentation framework $F$ is denoted $prf(F)$.\cl

\textbf{Example 4.} (Continuation of Example 3.) Since $\emptyset\subset\{a_2\}$, \(\emptyset\) is not a preferred extension. Since $\{a_1\}\subset\{a_1,a_2\}$ and $\{a_2\}\subset\{a_1,a_2\}$, $\{a_1\}$ and $\{a_2\}$ are not a preferred extensions. Since all other admissible sets are proper subsets of $\{a_1,a_2\}$, it is a preferred extension.\hl
It follows that $prf(F)=\{\{a_1,a_2\}\}$.\hl
As we can see, all arguments contained in admissible sets still are contained in a preferred extension, even though the number of results is smaller.\cl

\section{Stable extensions}

Additionally we can define a stricter version of the admissible set, that not only requires arguments to be defended, but also attacks all arguments not contained within it.\cl
...
\textbf{Definition 8.} Let $S$ be an admissible set. It is called a \emph{complete extension} if for each $a\not\in S$, $S$ does not defend $a$.\\
The set of all complete extensions of an argumentation framework $F$ is denoted $co(F)$.\cl

\textbf{Example 6.} (Continuation of Example 3.) The admissible set $\emptyset$ is not a complete extension because it defends $a_2$ which it doesn't contain. The same is true for $\{a_1\}$. $\{a_2\}$ defends $a_1$ and is therefore also not a complete extension.\\
...
As we have seen in the previous chapter, often there may be overlaps between the different extension types. In this chapter we look at the relations between extension types, explore their features and provide additional extension semantics. In advance the relations between extension types, including additional extension types, are shown in Figure 3.1 which was adapted from \cite{Gorogiannis}.\cl %erklären inwiefern?
...
Note that there was a slight change due to \cite{Baroni} stating that the ideal extension is always a superset of the grounded extension and \cite{Dung2} stating that an ideal extension is always a complete extension.
...
\begin{align}
	...
	prf(F)\not\subseteq st(F)
\end{align}
Therefore each stable extension also is an admissible set.\cl
...
All admissible sets except $\{c\}$ contain all the arguments they defend. Hence they are complete. $\{c\}$ defends $a$, which it doesn't contain and therefore is not complete. It follows that $co(F)=\{\emptyset,\{a\},\{d\},\{a,c\},\{a,d\},\{b,d\}\}$\hl
Of the admissible sets $\emptyset,\{a\}$ and $\{d\}$ are subsets of $\{a,d\}$, while $\{c\}$ is a subset of $\{a,c\}$. Those are not preferred extensions. We get $prf(F)=\{\{a,c\},\{a,d\},\{b,d\}\}$, since these sets are not subsets of each other.\hl
...
\input{modulartex/otherproperties.tex} %TODO use?
...
There also is an alternative semantic to the grounded extension. Ideal extension semantics were introduced within \cite{Alferes} and also produce a unique sceptical set, albeit a less strict one. They are defined in \cite{Baroni} as follows.\hl

\textbf{Definition 11.} Let $S$ be an admissible sets. It is called the \emph{ideal extension} if it is the maximal (wrt set inclusion) admissible set contained in every preferred extension.\\
...
\input{modulartex/furthertopics.tex} %TODO use?
...
Figure 4.4 shows the explanation shown during the computation of the complete extensions of the argumentation framework shown in Figure 4.1. Note that in this case, admissible sets (as a prerequisite to compute complete extensions) did not need to be computed, because they had been computed beforehand.
